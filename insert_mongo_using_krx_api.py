...
script_loc =  get_vault_configuration('crawler')['krx_api_script_loc']
...
default_args = {
    ...
    'start_date': datetime(2025, 3, 5, tzinfo=local_tz),
...
before_year = datetime.now(pytz.timezone('Asia/Seoul')).strftime('%Y')
before_date = (datetime.now(pytz.timezone('Asia/Seoul'))- timedelta(days=1)).strftime('%Y%m%d')
...
run_script_task = SSHOperator(
    ...
    command=f'{python_loc} {script_loc} --before_date="{before_date}"',  # 실행할 Python 스크립트 경로 입력
    environment={
...
def send_slack_message():
    ...
    data = {
        "text": f"insert_mongo_using_krx_api  is done at {current_time} "
    }
